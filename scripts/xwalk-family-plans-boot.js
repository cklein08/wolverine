...
(function () {
  ...
  const AI_BADGE = '✦ Personalized for You – Generated by AI';
  const SINGLE_OFFER = {
    'single-woman-nyc': {
      tagline:
  ...
  function buildSingleOfferPage(personaId, headline) {
    ...
    green.className = 'xwalk-mockup-green-bar';
    green.textContent = AI_BADGE;
    root.append(green);
    ...
    white.innerHTML +=
      ...
      meta.planPrice +
      '</span></h2><div class="xwalk-mockup-plan-pill"><p class="xwalk-plan-pill xwalk-plan-line-pill xwalk-plan-line-pill--dark">' +
      meta.pill +
  ...
  function build(section) {
    ...
    const headline = h1?.textContent?.trim() || 'Keep your family connected';
    const path = (location.pathname || '').replace(/\/$/, '');
    return buildSingleOfferPage(path.slice(1), headline);
  }
  ...
  function run() {
    ...
    if (main) {
      ...
      main.style.cssText = 'display:block;max-width:none;padding:0;background:#fff;';
    }
    const page = document.querySelector('.xwalk-family-plans-page');
    if (page?.querySelector('.xwalk-mockup-offer-row')) {
      const h1 = page.querySelector('.xwalk-family-headline, .xwalk-campaign-hero-headline');
    ...
    if (page?.querySelector('.xwalk-family-grid')) {
      ...
      const built = buildSingleOfferPage(personaId, h1?.textContent?.trim() || '');
    ...
    if (!section) return;
    if (section.querySelector('.xwalk-mockup-offer-row')) return;
